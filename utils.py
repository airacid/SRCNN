...
def input_setup( config):
  ...
  if config.is_train:
    ...
    data = prepare_data(dataset=config.sample_file)
  ...
  make_data( arrdata, arrlabel, config)
...
def prepare_data( dataset):
  ...
  if FLAGS.is_train:
    ...
    data = glob.glob(dataset)

  return data

def make_data( data, label, config):
  """
  Make input data as h5 file format
  Depending on 'is_train' (flag value), savepath would be changed.
  """
  if FLAGS.is_train:
    savepath = os.path.join('{}'.format(config.checkpoint_dir),
								"train.h5")
  else:
    savepath = os.path.join('{}'.format(config.checkpoint_dir),
								"test.h5")
